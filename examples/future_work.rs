...
fn parse_file(path: &PathBuf, color: bool) {
    ...
    if let Some(js) = get_js(path) {
        ...
        if let Ok(mut p) = Builder::new().js(js).with_comment_handler(ch) {
            let _ = p.parse();
        }
    }
}

fn get_js(path: &PathBuf) -> Option<String> {
...
impl CommentHandler for WorkHandler {
    fn handle_comment(&mut self, comment: Item) {
        match comment.token {
            Token::Comment(c) => if c.is_multi_line() {
                let mut counter = 0;
...
fn parse_line(span: Span, line: &str) -> Option<FutureWork> {
    let (todo, prefix) = if line.starts_with("TODO") {
        (true, "TODO")
    } else if line.starts_with("FIXME") {
        (false, "FIXME")
    } else {
        return None;
    };
